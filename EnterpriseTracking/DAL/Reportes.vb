    ...
    Public Function Obtener_Ventas_Cat_prod(ByVal desde As BE.Factura, ByVal hasta As BE.Factura) As List(Of BE.Rep_Vtas_prod_cat)
        ...
        Dim cadena As String = "select p.IDCategoria, c.Nombre NombreCategoria, fd.IDProducto , p.nombre as NombreProducto, sum(fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) cantidad, sum(fd.Precio * fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) Monto "
    ...
    End Function
        ...
        Dim params(2) As System.Data.SqlClient.SqlParameter
        Dim cadena As String = "select f.IDDistribuidor, c.Nombre NombreDistribuidor, f.IDVendedor , p.nombre as NombreVendedor, sum(fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) cantidad, sum(fd.Precio * fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) Monto "
        cadena += "from FacturaDetalle fd "
        cadena += "join factura f on f.id = fd.IDFactura "
        ...
        Try
            params(0) = DBH.CrearParametro("@P1", desde.Fecha)
            params(1) = DBH.CrearParametro("@P2", hasta.Fecha)
            params(2) = DBH.CrearParametro("@P3", desde.Distribuidor.Cliente.ID)
    ...
    Public Function Obtener_Ventas_TipoPDV(ByVal desde As BE.Factura, ByVal hasta As BE.Factura) As List(Of BE.Rep_Vtas_Dist_Vend)
        Dim params(2) As System.Data.SqlClient.SqlParameter
        Dim cadena As String = "select f.IDDistribuidor, c.Nombre NombreDistribuidor, 0 IDVendedor, p.TipoPDV as NombreVendedor, sum(fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) cantidad, sum(fd.Precio * fd.Cantidad * CASE WHEN f.TipoFactura='NC' THEN -1 else 1 END) Monto "
        cadena += "from FacturaDetalle fd "
        cadena += "join factura f on f.id = fd.IDFactura "
        cadena += "join PuntodeVenta p on p.id = f.IDPuntoVenta "
        cadena += "join Distribuidor c on c.id = f.IDDistribuidor "
        cadena += "where f.borrado = 0 "
        cadena += "and cast(Fecha as date) >= cast(@P1 as date) "
        cadena += "and cast(Fecha as date) <= cast(@P2 as date) "
        cadena += "and c.IDCliente = @P3 "
        cadena += "group by f.IDDistribuidor, c.Nombre ,  p.TipoPDV "
        cadena += "order by f.IDDistribuidor, c.Nombre , p.TipoPDV "
